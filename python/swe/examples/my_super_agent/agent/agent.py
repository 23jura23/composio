...
def get_agent_graph(repo_path: str, workspace_id: str):
    ...
    composio_toolset = ComposioToolSet(
        ...
        processors={
            ...
            "schema": {  # к каждой туле добавляется thought, чтобы модель подумала, а потом удаляется из ответа (то есть в тулу попадает только то что должно туда попасть, без thought).
